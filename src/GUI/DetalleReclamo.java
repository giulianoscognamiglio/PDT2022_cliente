...
public class DetalleReclamo extends JFrame {
	...
	public Usuario usuario = DAOGeneral.usuarioBean.obtenerPorId(reclamo.getEstudiante_id_usuario());
	...
	public DetalleReclamo() throws ServiciosException {
		...
		if (PanelMenu.usuarioIngresado instanceof Estudiante) {
			estudiante = DAOGeneral.estudianteBean.obtenerPorUsuario(PanelMenu.usuarioIngresado.getId_usuario());
		} else {
			estudiante = null;
		}
		...
		if (!(reclamo.getTipo().equals("Otro"))) {
			...
			LocalDate fechaInicioActividad = Instant.ofEpochMilli(reclamo.getFechaInicioActividad().getTime())
